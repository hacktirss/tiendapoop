...
$nameSession = "catalogoBitacora";
$arrayFilter = array();
$session = new PaginadorSession("b.id_bitacora", "b.id_bitacora", $nameSession, $arrayFilter, "Filtro");
...
$Id = 90;
$Titulo = "Bitacora de eventos y acceso";

$conditions = "b.cia = " . $usuarioSesion->getCia();
if ($Filtro !== "Todos") {
                        ...
                        <tr>
                            <td style="text-align: center">
                                <?php if (empty($session->getSessionAttribute("returnLink"))) : ?>
                                    <a href="<?= $cLink . "?busca=" . $row["id_bitacora"] ?>" data-id="<?= $row["id_bitacora"] ?>"><i aria-hidden="true" class="icon fa fa-lg fa-edit"></i></a>
                                <?php else: ?>
                                    <a href="<?= $rLink . "?busca=NUEVO&Bitacora=" . $row["id_bitacora"] ?>" data-id="<?= $row["id_bitacora"] ?>"><i aria-hidden="true" class="icon fa fa-lg fa-check"></i></a>
                                <?php endif; ?>
                            </td>
                            <?php echo $paginador->formatRow(); ?>
                            <td style="text-align: center">
                                <?php if (empty($session->getSessionAttribute("returnLink"))) : ?>
                                    <a href=javascript:borrar("<?= $row["id_bitacora"] ?>","<?= $self ?>"); data-id="<?= $row["id_bitacora"] ?>"><i aria-hidden="true" class="icon fa fa-lg fa-trash"></i></a>
                                    <?php endif; ?>
                            </td>                            
                        </tr>
                    <?php } ?>

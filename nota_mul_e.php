...
$arrayFilter = array();
$session = new PaginadorSession("ne.id", "ne.id", $nameSession, $arrayFilter, "Filtro");
...
$Id = 99;
$Titulo = "Notas de entrada multiple";

$conditions = "ne.cia = " . $usuarioSesion->getCia();
...
$paginador = new Paginador($Id,
        "",
        "LEFT JOIN prv ON ne.proveedor = prv.id AND ne.cia = prv.cia ",
        "",
        "$conditions",
                    ...
                    while ($paginador->next()) {
                        ...
                        <tr>
                            <td class="centrar">
                                ...
                                <?php else: ?>
                                    <a href="<?= $rLink . "?busca=NUEVO&EntradaMul=" . $row["id"] ?>" data-id="<?= $row["id"] ?>"><i aria-hidden="true" class="icon fa fa-lg fa-check"></i></a>
                                <?php endif; ?>
                            </td>
                            <td class="centrar">
                                <?php if ($row["status"] === "Cerrada") : ?>
                                    <a href=javascript:winuni("notam.php?busca=<?= $row["id"] ?>");><i class="icon fa fa-lg fa-print"></i>
                                    <?php endif; ?>
                            </td>

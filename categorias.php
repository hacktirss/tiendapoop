...
$arrayFilter = array();
$session = new PaginadorSession("cat.id", "cat.id", $nameSession, $arrayFilter, "Filtro");
...
$conditions = "cat.cia = " . $usuarioSesion->getCia();
if ($Filtro !== "Todos") {
    ...
    <body>
        ...
        <div id="TablaDatos">
            <table aria-hidden="true">
                <?php echo $paginador->headers(array(empty($session->getSessionAttribute("returnLink")) ? "Editar" : "Seleccionar","Sub-Categorias"), array(empty($session->getSessionAttribute("returnLink")) ? "Borrar" : "")); ?>
                <tbody>
                    <?php
                    while ($paginador->next()) {
                        ...
                        <tr>
                            <td style="text-align: center">
                                ...
                                <?php else: ?>
                                    <a href="<?= $rLink . "?busca=NUEVO&Categoria=" . $row["id"] ?>" data-id="<?= $row["id"] ?>"><i aria-hidden="true" class="icon fa fa-lg fa-check"></i></a>
                                <?php endif; ?>
                            </td>
                            <td style="text-align: center">
                                <?php if (empty($session->getSessionAttribute("returnLink"))) : ?>
                                    <a href="<?= $cLinkd . "?criteria=ini&" . DETALLE . "=" . $row["id"] ?>" data-id="<?= $row["id"] ?>"><i aria-hidden="true" class="icon fa fa-lg fa-list-alt"></i></a>
                                    <?php endif; ?>
                            </td>
